...
class FinancialStatementAnalyzer:
    ...
    def prepare_data(self):
        ...
        self.df['debit'] = self.df.apply(
            lambda row: row['amount'] if row['dr_cr_indicator'] == 'DR' else 0, axis=1
        )
        self.df['credit_amount'] = self.df.apply(
            lambda row: row['amount'] if row['dr_cr_indicator'] == 'CR' else 0, axis=1
        )
        
        # Handle the 'credit' column which seems to contain transaction descriptions
        self.df['description'] = self.df['credit'].fillna('')
    ...
    def get_monthly_expenses(self) -> Dict[str, float]:
        ...
        for month_year, group in self.df.groupby('month_year'):
            month_debits = group[group['dr_cr_indicator'] == 'DR']['amount'].sum()
    ...
    def get_monthly_surplus(self) -> Dict[str, Dict[str, float]]:
        ...
        for month_year, group in self.df.groupby('month_year'):
            credits = group[group['dr_cr_indicator'] == 'CR']['amount'].sum()
            debits = group[group['dr_cr_indicator'] == 'DR']['amount'].sum()
    ...
    def detect_emi_payments(self) -> List[Dict[str, Any]]:
        ...
        for _, row in debit_transactions.iterrows():
            ...
            similar_transactions = debit_transactions[
                abs(debit_transactions['amount'] - amount) / amount <= 0.05
            ]
            ...
            if len(similar_transactions) >= 2:
                ...
                # Check for regular pattern (monthly)
